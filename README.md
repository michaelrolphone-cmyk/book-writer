...
> Integration note: this release does not add or remove CLI commands or GUI API endpoints; existing command and endpoint contracts listed below remain valid. It updates internal Survey Sketch canvas→state-plain conversion so X/Y ratios are mirrored by default to match map orientation (with optional `invertX`/`invertY` extent flags when needed) while preserving pan/zoom scale + offset handling, preventing viewport movement from appearing diagonally inverted.
- `GET /` (GUI HTML)
...
- Full-book narration uses the first chapter's sample rate for silence gaps and resamples chapters to keep ffmpeg concatenation stable.
...
- `--cover-command`: Custom command template for cover generation (uses placeholders like `{prompt}` and `{output_path}`); the default Swift command uses `--step-count` for the inference steps and does not pass width/height flags. If you supply a Swift `StableDiffusionSample` command that includes `--width`/`--height`, the CLI will strip them when the tool help output does not list those options (use a custom command with `--steps`, `--width`, or `--height` if your local build supports them).
...
- Taxonomy/journey generation chunks long book content to stay within model context limits, assembles the taxonomy additively, and appends journey steps sequentially across chunks; the JSON parser tolerates Markdown code fences or extra text around the returned JSON.
