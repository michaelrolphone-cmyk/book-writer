...
class TestGui(unittest.TestCase):
    def test_get_gui_html_contains_key_sections(self) -> None:
        ...
        self.assertIn("id=\"nowPlayingInfo\"", html)
        ...
        self.assertIn("id=\"nowPlayingSettings\"", html)
        ...
        self.assertIn("id=\"bookWorkspaceDownloads\"", html)
        self.assertIn("id=\"bookWorkspaceGenerateMenu\"", html)
        ...
        self.assertIn("invertX: extent.invertX !== undefined ? Boolean(extent.invertX) : true,", html)
        self.assertIn("invertY: extent.invertY !== undefined ? Boolean(extent.invertY) : true,", html)
        self.assertIn("const mappedRatioX = canvasExtent.invertX ? 1 - boundedRatioX : boundedRatioX;", html)
        self.assertIn("const mappedRatioY = canvasExtent.invertY ? 1 - boundedRatioY : boundedRatioY;", html)
        ...
        self.assertIn("y: point.y,", html)
        self.assertIn("const handoffPointforgeToSurveySketch = (points, transform = {}) => ({", html)
        self.assertIn("points: Array.isArray(points) ? points : [],", html)
        self.assertIn("transformLayer: createSurveySketchTransformLayer(transform),", html)
        self.assertIn("const statePlainToLatLon = (point, options = {}) => {", html)
        self.assertIn("const converter = options.converter || window.proj4;", html)
        self.assertIn("const [lon, lat] = converter(fromProjection, toProjection, [x, y]);", html)
        self.assertIn("const buildMapViewportFromStatePlainPoints = (points, options = {}) => {", html)
        self.assertIn("const syncSurveySketchMapViewport = (map, points, options = {}) => {", html)
        self.assertIn("map.fitBounds(viewport.bounds, options.fitOptions || { padding: [24, 24] });", html)
        ...
        self.assertIn("nowPlayingInfo.addEventListener('click'", html)
        ...
        self.assertIn(".icon-button {", html)
        self.assertIn(".edit-link {", html)
        ...
        self.assertIn(".chapter-card .book-cover-title {", html)
