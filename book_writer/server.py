...
def _get_book_folder_created(book_dir: Path) -> float | None:
    try:
        stat_result = book_dir.stat()
    except OSError:
        return None
...
def _parse_cover_settings(payload: dict[str, Any]) -> CoverSettings:
    ...
    return CoverSettings(
        ...
        width=parse_int(cover_payload.get("width", 2560), 2560),
        height=parse_int(cover_payload.get("height", 1600), 1600),
        ...
        entries.append(
            {
                ...
                "folder_created": _get_book_folder_created(book.path),
                "cover_url": (
